...
管理端注册码列表、使用者查询、公开校验和写入前置检查会先刷新这份单一状态文档，再从内存快照读取。这样即使运行在 PostgreSQL state、另一个进程刚做过维护，或管理员直接修复 JSON state，删除 / 修改后的注册码也不会继续从旧缓存里显示或被使用。WebUI 的注册码读接口也会绕过短读缓存和 in-flight GET 合并。
