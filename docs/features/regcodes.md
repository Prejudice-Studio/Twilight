...
注册码与全部业务状态一样，保存在「单一状态文档」中——唯一运行后端是 PostgreSQL，整份状态是 `twilight_state` 表（`id=1` 的一行 `jsonb`）。`internal/store` 中 `RegCode` 以 `code` 字符串为键存放于状态文档的 `RegCodes` 映射里，**不存在**独立的注册码数据库文件或单独的表，也没有「建表 / ALTER TABLE」之类的迁移步骤。

管理端注册码列表、使用者查询、公开校验和写入前置检查会先刷新这份单一状态文档，再从内存快照读取。这样即使另一个进程刚写过 `twilight_state`、或管理员直接在库里修复了状态，删除 / 修改后的注册码也不会继续从旧缓存里显示或被使用。WebUI 的注册码读接口也会绕过短读缓存和 in-flight GET 合并。
...
最终写入时 `UpsertRegCodes` 仍会在 store 全局锁下重新检查最新持久化状态。批量生成是 create-only 语义：同一批内重复或持久化状态中已经存在同名卡码时，整批返回冲突，不会覆盖旧卡码，也不会出现半批写入。
...
运行后端已收敛为单一 PostgreSQL：运行期 store 恒为 postgres，唯一的失配场景是配置里的 `database.driver` 被改成了非 postgres 值（`runtimeDatabaseMismatch`，见 `internal/api/storage_guard.go`；这本应在启动期被拒，此处作为运行期最后一道保险）。失配时所有注册码「写入类」操作（创建、更新、删除、批量删除、清理使用、以及通过 use-code/renew 消费注册码）都会被拦截，返回 `409` + `REGCODE_STORAGE_MISMATCH`，提示把 `database.driver` 改回 postgres 并重启后再操作，以免写到错误的存储里。

上述护栏之外，注册码读路径与写入前置读取会主动刷新 store 状态；如果刷新失败，接口返回 500 并提示读取最新数据库状态失败，而不是继续用旧快照执行删除、启停或消费。
