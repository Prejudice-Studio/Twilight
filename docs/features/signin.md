...
管理员开启全局许可后，符合条件的用户可在签到页的积分续期区域开启「到期自动续期」。V2 前端通过 `PUT /api/v2/signin/preferences`（兼容入口仍为 `PUT /api/v1/users/me`）写入严格布尔字段：
...
V2 签到页的读取使用 `GET /api/v2/signin/summary`，签到和积分续期分别使用 `POST /api/v2/signin` 与 `POST /api/v2/signin/renew`。这些接口返回私有 `no-store` 响应，页面写入成功后重新由 SSR 读取权威摘要。
