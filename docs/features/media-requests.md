...
管理员求片队列的默认前端位于 `webui-v2/src/routes/(app)/admin/requests`，页面标题和筛选/队列容器复用无状态 SSR `PageHeader` / `Panel` 组件。队列的来源样式、同名聚合与显式拆分、revision 并发保护和所有写入 action 保持原有后端契约；共享组件迁移不应把完整请求历史或浏览器端状态重新引入首屏。
