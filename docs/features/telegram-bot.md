...
- Bot 仍按 Telegram `getUpdates` 长轮询顺序处理更新，保证同一会话中的绑定、删除确认、内联按钮和自定义指令交互不会乱序。
...
- 群成员花名册仍记录首次出现、最近出现和成员状态，但普通群消息对未变化成员的 `LastSeen` 最多每五分钟持久化一次。成员加入、退出、权限状态变化和首次识别为 Bot 仍会立即落库，避免把每条群消息放大为整份状态文档写入。
- 更新处理保持串行，批次全部处理完成后才持久化 offset。进程异常时可能重取尚未确认的批次，但不会为了追求并发速度而提前确认有副作用的命令。
