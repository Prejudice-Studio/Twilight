...
- `getUpdates` 等 JSON API 通过统一的类型化协议层直接解码 `result`，不再先读取完整响应、保留 `RawMessage` 后二次解析。写操作使用固定请求结构并丢弃无用结果；成功响应限制为 4 MiB，调用方已有 deadline 不会被 20/45 秒默认值覆盖，HTTP 429 的 `retry_after` 退避语义保持不变。
...
- 自定义指令和禁用指令会在配置快照上建立只读索引，配置热重载后自动重建，监听路径无需逐条扫描配置列表。
...
当前 100 条中等文本更新的本地解码基准中，旧双重解析路径约为 `697-743 us/op`、`453304 B/op`，类型化单次解析约为 `418-450 us/op`、`322065 B/op`；耗时下降约 35%-40%，临时内存下降约 29%。基准用于防止热路径回退，具体绝对值会随硬件和 Go 版本变化。
...
Telegram 绑定码只是短期运行时票据，真正的账号绑定以用户记录中的 `telegram_id` 为准。历史版本曾把绑定码状态持久化到状态文档，可能留下"绑定码已确认，但没有对应用户记录"的残留。当前版本在系统启动和配置热重载时会自动清理这些旧版持久化绑定码，并清理内存中过期的绑定码；这不会解绑已有用户，也不会删除 Telegram 群成员花名册。
