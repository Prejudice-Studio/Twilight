...
| `/delAccount [原因]` | 删除自己的账号。验证优先级：已绑定邮箱→邮箱验证码；已绑定 Emby→Web密码+Emby密码两步验证；无绑定→直接确认。可附带可选删除原因（记录到审计日志）。禁用状态的 Web/Emby 账号默认不可自删；邀请关系中的已到期账号如果 Emby 已被到期任务禁用，可用 Web 密码完成退出清理，执行时会完全删除对应 Emby 账号。示例：`/delAccount 不再使用本服务` 或 `/delAccount emby`。 |
