...
- `getUpdates` 直接解码为 `telegramUpdate` / message / callback / chat-member 窄 DTO，不再构造嵌套 `map[string]any`。排序、花名册、inline、JS callback 和命令分发都读取强类型字段；批次分组仅保存原切片索引并向 worker 传指针，不复制 update 值。普通回复丢弃无需使用的 Telegram Message 结果，只有需要 `message_id` 的面板/交互发送才解析返回对象。
