...
用户通知模板 `Ticket.notify_telegram_template` 仍可自定义。默认模板是 HTML；自定义模板可用纯文本，若混入裸 `<` / `&` 导致 Telegram 解析失败，会自动降级为纯文本重发，保证消息始终送达。通知发送是对外副作用而非状态变更：成功 / 跳过只写 Debug 运行日志（默认 Info 级不落盘），失败写 Warn，均不进审计库。Telegram 未配置时不会启动通知协程。
