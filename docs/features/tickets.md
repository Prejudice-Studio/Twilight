...
用户创建工单成功后，后端会先完成 store 持久化，再写入 `create_ticket` 审计日志并发送 Telegram 管理员通知；运行日志会记录不含正文的 `ticket_id`、提交人、类型和优先级，便于排查“通知已到但后台未显示”的问题。WebUI 的用户工单列表、管理端工单列表和审计日志列表都绕过前端短读缓存，手动刷新应直接读取后端当前状态。
