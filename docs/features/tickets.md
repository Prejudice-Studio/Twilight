...
`replies` 是双方对话历史的唯一来源。`admin_note` 是仅管理端可见的内部处理摘要；修改它不会生成回复，也不能用它覆盖或清空历史回复。管理员对用户发送的文字必须走专用回复接口并追加到 `replies`。
...
管理端工单处理页支持点击单个工单进入会话式详情页。详情页通过 `GET /admin/tickets/{ticket_id}` 读取完整对话，通过 `POST /admin/tickets/{ticket_id}/reply` 单独追加管理员文字回复；图片上传、预览和删除复用 `/tickets/{ticket_id}/images`，受全局工单图片大小和数量限制。V1 支持粘贴图片，WebUI 详情页通过 `apiRequestForm` 提交同源 multipart 请求，后端规则不变。
...
用户新建、回复、上传图片、关闭或重开工单时，会通知已绑定 Telegram 且开启工单通知的管理员。管理员处理工单时仍可通知其他订阅管理员用于协作，但不会把操作者自己的处理动作再推送给操作者本人。工单变动也会按工单归属向对应用户推送（用户可在个人设置或单条工单上关闭）。

通知统一使用 HTML 富文本排版，不受全局 `Telegram.parse_mode` 影响：带 emoji 图标的标题、分隔线、加粗字段标签，状态（🟡 待处理 / 🔵 处理中 / 🟢 已解决 / ⚫ 已关闭）和优先级（🟢 低 / 🟡 中 / 🟠 高 / 🔴 紧急）都翻译成带图标的中文，最新回复 / 内容摘要用引用块展示。所有动态字段（标题、用户名、摘要、备注）在拼入 HTML 前都会转义，用户可控内容不会破坏排版或注入标签。图片上传通知会把富文本 caption 随图片一起发送。

用户通知模板 `Ticket.notify_telegram_template` 仍可自定义。默认模板是 HTML；自定义模板可用纯文本，若混入裸 `<` / `&` 导致 Telegram 解析失败，会自动降级为纯文本重发，保证消息始终送达。通知发送是对外副作用而非状态变更：成功 / 跳过写 Debug 运行日志且不刷默认控制台，失败写 Warn，均不进审计库；运行日志面板仍会保存这些级别用于排障。Telegram 未配置时不会启动通知协程。
