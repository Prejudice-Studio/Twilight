...
- 关键架构约定（主要业务状态单文档模型、唯一运行后端 PostgreSQL，以及 `twilight_audit_logs`/`twilight_sessions`/`twilight_runtime_logs`/`twilight_playback_records`/`twilight_telegram_runtime` 独立表、配置热重载、CORS 与鉴权边界等）见 [开发指南](./guides/development.md) 与 [安全加固](./guides/security.md)。
