...
- 配置管理页的 TOML、schema 读取和各业务页嵌入的 `AdminConfigSections` 编辑器都使用可取消请求；切换页签、重新加载或离开页面时，旧响应不能覆盖当前编辑状态。配置源与备份预览使用 Firefox 兼容的受限 `dvh` 滚动区域。
- 设备/IP 表格、求片处理队列、工单会话与附件条、注册码使用记录和运行日志必须在自身的 `dvh` Firefox 滚动区域中显示。Flex 工单会话正文要保留 `min-h-0`，否则窄屏时 Firefox 可能让内容撑开容器而不是内部滚动；长表格固定表头并在自身处理横向滚动。
