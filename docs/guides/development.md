...
- API 响应解析由 `webui/src/lib/api-request.ts` 通过有界流读取，最大接受 8 MiB 的响应体；读取过程中超过上限会取消流并拒绝解析，防止异常响应把浏览器内存一次性推高。请求超时 signal 会持续到正文读取结束，避免只收到响应头后就解除超时保护。错误诊断必须带实际请求版本（`/api/v1` 或 `/api/v2`），不能把 V2 请求误报为 V1。
