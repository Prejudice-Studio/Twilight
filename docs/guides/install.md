...
另有九张独立表：`twilight_audit_logs`（操作审计）、`twilight_sessions`（会话）、`twilight_runtime_logs`（后台实时日志）、`twilight_playback_records`（旧版兼容播放记录）、`twilight_playback_events`（可信观看原始事件）、`twilight_playback_segments`（播放段状态）、`twilight_playback_daily`（可重建日桶）、`twilight_telegram_roster`（Telegram 群成员花名册）、`twilight_telegram_runtime`（Telegram 更新确认游标）。这些表会由后端幂等创建，无需手工执行迁移 SQL；升级时旧主状态中的花名册会自动迁移并从 JSONB 清除。
