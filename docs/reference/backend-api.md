...
V2 基础协议目前提供 `GET /api/v2/system/health`、`GET /api/v2/system/capabilities` 和受保护的 `GET /api/v2/dashboard/summary`。它们沿用统一 JSON envelope；仪表盘摘要一次返回当前用户、公开能力和在线人数状态。Emby 读取失败时只将 `data.viewers.available` 设为 `false`，本地用户和能力数据仍然返回，不把故障伪装为零人在线。未迁移调用继续使用 `/api/v1`。
