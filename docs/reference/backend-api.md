...
| `GET /emby/playback-stats` | `AuthUser` | Current-user Emby ActivityLog playback stats; supports `days`, `today=1`, `limit`, `sort`, `refresh=1` |
| `GET /admin/emby/activity-logs` | `AuthAdmin` | Raw Emby activity logs; throttled auto refresh by default, `refresh=1` forces fetch |
| `GET /admin/emby/playback-stats` | `AuthAdmin` | Playback stats for global, self, or selected user scopes; supports `scope=self|global|user`, `uid`, `days`, `today=1`, `limit`, `sort`, `refresh=1` |
| `GET /admin/emby/playback-stats/{uid}` | `AuthAdmin` | Emby ActivityLog playback stats for the specified UID |
