...
| GET | `/api/v2/emby/play-rank` | Public | V2 播放日榜/周榜（用户名脱敏、不下发 UID）；路由是 AuthPublic，是否放行由 `play_rank_enabled` / `play_rank_user_visible` / `play_rank_anonymous` 在 handler 内判断 |
