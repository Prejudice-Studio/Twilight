...
| GET | `/api/v2/admin/emby/play-rank` | Admin | V2 播放榜单（含 UID 与完整用户名）；`range=day\|week\|month\|all`、`days=N`、`group_by=item\|series`、`sort_by=plays\|duration`、`limit`、`refresh=1`，60 秒缓存，活动日志同步写入后自动失效 |
| GET | `/api/v2/emby/play-rank` | User | V2 播放榜单（用户名脱敏、不下发 UID）；参数同上，`AuthUser`，未登录一律拒绝，普通用户能否查看由 `play_rank_enabled` / `play_rank_user_visible` 决定 |
