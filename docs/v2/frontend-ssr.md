...
4. 同源 API 代理只接受 `/api/v1/*` 与 `/api/v2/*`。构造上游 URL 前会拒绝原始或百分号编码的 `.` / `..` 路径段、反斜杠、非法编码和片段标记；查询参数仍可用于服务端筛选，但 URL 规范化不能把请求带出 `/api/` 命名空间。
5. 登录 action 只接受配置的会话 Cookie（默认 `twilight_session`），解析上游 `Set-Cookie` 时忽略后端 Domain，只向当前 V2 站点写 host-only、HttpOnly Cookie，避免跨域扩大 Cookie 作用范围。
6. SSR 读取默认为 `no-store`。当前用户身份、权限和会话状态不能进入共享浏览器缓存。
...
邮箱找回的验证码申请与确认通过 `email_password_reset_service.go` 执行；申请阶段对不存在邮箱保持统一成功文案，确认阶段在验证码、账号状态和密码强度全部通过后才更新密码并撤销旧会话。
