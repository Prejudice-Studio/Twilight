...
- 页面结构基线由无状态的 `PageHeader.svelte` 与 `Panel.svelte` 提供。管理员的工单、用户、求片、注册码、邀请和 Emby 页面已经按模块使用该基线；这些组件只负责语义标题、内容边界和响应式换行，业务状态、危险操作、权限与外部副作用仍留在各自的 SSR route/action 和 Go 后端。
...
- Emby 找回密码由 `internal/api/password_reset_service.go` 负责外部鉴权、本地账号/资格校验、临时密码生成、哈希更新和会话撤销；V1 与 V2 不得各自复制这条状态转移。
- 邮箱找回密码由 `internal/api/email_password_reset_service.go` 负责验证码发放/核验、账号状态与密码强度校验、哈希更新和会话撤销；邮箱不存在时继续保持防枚举的统一响应。
...
状态机的单元测试覆盖暂停/恢复不计入空档、客户端时间未来偏移限制、播放段总时长上限、跨本地日切桶、无效时区和乱序序列。数据库事件表、原子幂等写入、每日聚合查询和 V2 路由仍需在该领域边界之上实现，不能把这些规则复制到 HTTP handler。
