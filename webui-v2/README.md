...
目前已实现 SSR 壳层、登录/登出、服务端会话读取、V2 capabilities 读取、在线人数摘要和仅限 `/api/v1` / `/api/v2` 的同源代理。其它 V1 功能必须按照 [V2 SSR 前端迁移规则](../docs/v2/frontend-ssr.md)逐模块迁移后才能声明支持。
...
- API JSON 响应有 8 MiB 读取上限，代理请求体同样有界。
