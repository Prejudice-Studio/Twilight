...
目前已实现 SSR 壳层、登录/注册/登出、服务端会话读取、个人设置（偏好、邮箱验证、密码安全、Emby 绑定）、公告、在线人数摘要、签到与积分、用户邀请中心、用户工单、Bangumi 用户端、求片中心和管理员服务器状态页，以及仅限 `/api/v1` / `/api/v2` 的同源代理。Bangumi 首屏使用 `/api/v2/bangumi/summary` 聚合本地同步状态、公开账号资料和有限收藏预览；收藏详情使用服务端分页，Token 只留在服务端请求链路中。管理员状态页由管理员服务端布局保护，三个健康检查独立读取并部分降级；其它 V1 功能必须按照 [V2 SSR 前端迁移规则](../docs/v2/frontend-ssr.md)逐模块迁移后才能声明支持。
