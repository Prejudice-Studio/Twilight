...
export const actions: Actions = {
  signin: async (event) => mutate(event, "/api/v1/signin", "POST", {}, "signin", "签到失败，请稍后重试"),
  renew: async (event) => mutate(event, "/api/v1/signin/renew", "POST", {}, "renew", "积分续期失败，请稍后重试"),
  autoRenewal: async (event) => {
    const form = await event.request.formData();
    const enabled = text(form, "enabled") === "true";
    return mutate(event, "/api/v1/users/me", "PUT", { signin_auto_renewal: enabled }, "auto-renewal", "自动续期设置失败，请稍后重试");
  }
};
