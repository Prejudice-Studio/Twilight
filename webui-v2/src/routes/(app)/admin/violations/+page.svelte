...
<section class="violations-page" aria-labelledby="violations-title">
  <PageHeader id="violations-title" eyebrow={t.adminArea} title={t.adminViolationsTitle} description={t.adminViolationsDescription}>
    {#snippet actions()}
...
<style>
  ...
  .panel-heading, .entry-header, .identity, .metadata, .pagination { align-items: flex-start; display: flex; gap: .75rem; }
  .panel-heading, .entry-header, .pagination { justify-content: space-between; }
  ...
  @media (max-width: 600px) { .panel-heading, .entry-header { align-items: stretch; flex-direction: column; } .clear-form, .clear-form .button { width: 100%; } .filter-form { grid-template-columns: 1fr; } .search-field { grid-column: auto; } .violation-entry { flex-direction: column; } .delete-form, .delete-form .button { width: 100%; } .entry-header time { max-width: none; text-align: left; } .pagination { align-items: stretch; } .pagination .button { min-width: 0; } }
</style>
