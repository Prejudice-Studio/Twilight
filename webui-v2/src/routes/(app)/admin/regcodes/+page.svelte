...
<section class="regcodes-page" aria-labelledby="regcodes-title">
  <PageHeader id="regcodes-title" eyebrow={t.adminArea} title={t.adminRegcodesTitle} description={t.adminRegcodesDescription}>
    {#snippet actions()}
  ...
  {#if action.action === "created" && action.codes?.length}
    <Panel id="generated-title" className="generated-panel" title={t.adminRegcodesGeneratedCodes}>
      <div class="code-output">{#each action.codes as code}<code>{code}</code>{/each}</div>
      <p class="muted">{t.adminRegcodesCreateDescription}</p>
    </Panel>
  {/if}

  <Panel id="create-title" title={t.adminRegcodesCreate} description={t.adminRegcodesCreateDescription}>
...
<style>
  ...
  .entry-header, .identity, .metadata, .entry-actions, .batch-toolbar, .pagination { align-items: flex-start; display: flex; gap: .75rem; }
  .entry-header, .pagination { justify-content: space-between; }
  p { overflow-wrap: anywhere; } .muted { color: #52606d; margin: .4rem 0 0; }
