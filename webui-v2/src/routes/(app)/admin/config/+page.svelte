...
<section class="config-page" aria-labelledby="config-title">
  <PageHeader id="config-title" eyebrow={t.adminArea} title={t.adminConfigTitle} description={t.adminConfigDescription}>
    {#snippet actions()}
      <a class="back-link" href="/admin/status">{t.adminStatusTitle}</a>
      <a class="button secondary" href="/admin/config">{t.adminConfigRefresh}</a>
    {/snippet}
  </PageHeader>
  ...
  {:else if activeTab === "toml"}
    <Panel id="toml-heading" className="toml-panel">
      <header class="panel-heading"><div><h2 id="toml-heading">{t.adminConfigToml}</h2><p class="muted">{t.adminConfigTomlHelp}</p></div></header>
...
<style>
  ...
  .panel-heading, .form-footer, .toolbar, .upload-form, .backup-actions { align-items: flex-start; display: flex; gap: .75rem; }
  .panel-heading { justify-content: space-between; }
  .toolbar, .backup-actions { align-items: center; flex-wrap: wrap; }
  ...
  @media (max-width: 560px) { .panel-heading, .form-footer, .upload-form, .backup-row { align-items: stretch; flex-direction: column; } .form-footer .button, .upload-form .button { width: 100%; } .workspace .section-nav, .editor-column { padding: .85rem; } .field-heading { align-items: stretch; flex-direction: column; } .secret-control, .backup-actions { align-items: stretch; flex-direction: column; } .secret-control .button, .backup-actions form, .backup-actions .button { width: 100%; } .command-row { grid-template-columns: 1fr; } .icon-button { width: 100%; } }
</style>
