...
<section class="database-page" aria-labelledby="database-title">
  <header class="page-heading">
    <div><p class="eyebrow">{t.adminArea}</p><h1 id="database-title">{t.adminDatabaseTitle}</h1><p class="muted">{t.adminDatabaseDescription}</p></div>
    <div class="heading-actions"><a class="back-link" href="/admin/status">{t.adminStatusTitle}</a><a class="button secondary" href="/admin/database">{t.adminDatabaseRefresh}</a></div>
  </header>
  ...
  <section class="panel status-panel" aria-labelledby="status-heading">
    ...
    {#if data.status}
      <dl class="facts"><div><dt>{t.adminDatabaseRuntime}</dt><dd>{valueLabel(data.status.active_label || data.status.active_driver)}</dd></div><div><dt>{t.adminDatabaseConfigured}</dt><dd>{valueLabel(data.status.configured_label || data.status.configured_driver)}</dd></div><div><dt>{t.adminDatabaseUserCount}</dt><dd>{data.status.user_count.toLocaleString("zh-CN")}</dd></div><div><dt>{t.adminDatabaseBackupCount}</dt><dd>{data.status.backup_count.toLocaleString("zh-CN")}</dd></div><div><dt>{t.adminDatabaseRedis}</dt><dd>{valueLabel(data.status.redis_enabled)}</dd></div><div><dt>{t.adminDatabaseStateFile}</dt><dd>{data.status.state_file || "-"}</dd></div></dl>
  ...
  <section class="panel" aria-labelledby="backups-heading">
...
<style>
  ...
  .page-heading, .heading-actions, .panel-heading, .backup-create, .backup-row, .backup-actions, .migration-form, .result-heading { align-items: flex-start; display: flex; gap: .75rem; }
  .page-heading, .panel-heading, .backup-row, .result-heading { justify-content: space-between; } .page-heading { border-bottom: 1px solid #d7dee5; padding-bottom: 1.15rem; } .heading-actions, .backup-actions { align-items: center; flex-wrap: wrap; } .eyebrow { color: #486581; font-size: .8rem; font-weight: 700; letter-spacing: .08em; margin: 0 0 .45rem; text-transform: uppercase; }
  ...
  .panel { background: #fff; border: 1px solid #d7dee5; border-radius: .45rem; min-width: 0; padding: 1rem; } .status-chip { background: #eef2f4; border: 1px solid #c8d2da; border-radius: 999px; color: #486581; flex: 0 0 auto; font-size: .8rem; padding: .25rem .55rem; white-space: nowrap; }
  ...
  @media (max-width: 560px) { .page-heading, .heading-actions, .panel-heading, .result-heading { align-items: stretch; flex-direction: column; } .heading-actions, .heading-actions > *, .heading-actions .button { width: 100%; } .facts { grid-template-columns: 1fr; } .panel { padding: .85rem; } .backup-actions { flex-direction: column; } .backup-actions .button { width: 100%; } }
</style>
