...
<section class="requests-page" aria-labelledby="requests-title">
  <PageHeader id="requests-title" eyebrow={t.adminArea} title={t.adminRequestsTitle} description={t.adminRequestsDescription}>
    {#snippet actions()}
  ...
  <Panel id="request-filter-title" className="filters" title={t.adminRequestsFilters} description={t.adminRequestsTotal.replace("{count}", String(data.payload?.request_total || 0))}>
    <span class="count">{data.payload?.total || 0} {t.adminRequestsGroups}</span>
  ...
  <Panel id="request-list-title" className="list-panel" title={t.adminRequestsQueue} description={t.adminRequestsPageOf.replace("{page}", String(data.payload?.page || data.query.page)).replace("{pages}", String(Math.max(1, data.payload?.total_pages || 1)))}>
...
<style>
  ...
  .title-row, .meta, .request-main, .request-actions, .update-form, .pagination, .group-banner { align-items: flex-start; display: flex; gap: .75rem; }
  h3, p { overflow-wrap: anywhere; } h3 { margin: 0; font-size: 1.05rem; }
  .meta, .overview, .note, .admin-note { color: #52606d; margin: .35rem 0 0; } .meta { flex-wrap: wrap; font-size: .8rem; }
  ...
  .status-tabs { display: flex; gap: .35rem; overflow-x: auto; overscroll-behavior: contain; scrollbar-color: #9fb3c8 #eef2f4; scrollbar-width: thin; } .status-tabs a { color: #36566a; min-height: 2.4rem; padding: .55rem .7rem; text-decoration: none; white-space: nowrap; } .status-tabs a.active { border-bottom: 3px solid #245b75; color: #16394a; font-weight: 700; } .status-tabs span { color: #52606d; font-size: .75rem; }
