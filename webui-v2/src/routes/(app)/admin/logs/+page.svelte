...
<section class="logs-page" aria-labelledby="logs-title">
  <PageHeader id="logs-title" eyebrow={t.adminArea} title={t.adminRuntimeLogsTitle} description={t.adminRuntimeLogsDescription}>
    {#snippet actions()}
      <a class="text-link" href="/admin/status">{t.adminStatusTitle}</a>
      <a class="button primary" href={`/admin/logs?limit=${data.limit}`}>{t.adminRuntimeLogsRefresh}</a>
    {/snippet}
  </PageHeader>
  ...
  <Panel id="stream-title" className="log-panel">
  ...
  {#if data.status}
    <div class="detail-grid">
      <Panel id="runtime-title" title={t.adminRuntimeLogsGoRuntime}><dl class="facts"><div><dt>{t.adminRuntimeLogsVersion}</dt><dd>{statValue(data.status.go_version)}</dd></div><div><dt>{t.adminRuntimeLogsPlatform}</dt><dd>{statValue(`${data.status.goos}/${data.status.goarch}`)}</dd></div><div><dt>{t.adminRuntimeLogsGoroutines}</dt><dd>{statValue(data.status.goroutines)}</dd></div><div><dt>{t.adminRuntimeLogsCPU}</dt><dd>{statValue(data.status.cpu_count)}</dd></div></dl></Panel>
      <Panel id="service-title" title={t.adminRuntimeLogsServiceStatus}><dl class="facts"><div><dt>{t.adminRuntimeLogsStartedAt}</dt><dd>{dateLabel(data.status.started_at)}</dd></div><div><dt>{t.adminRuntimeLogsRedis}</dt><dd>{statValue(data.status.redis_enabled)}</dd></div><div><dt>{t.adminRuntimeLogsLevel}</dt><dd>{statValue(data.status.log_level)}</dd></div><div><dt>{t.adminRuntimeLogsBackend}</dt><dd>{statValue(data.status.runtime_log_backend || data.status.active_database)}</dd></div><div><dt>{t.adminRuntimeLogsBuffer}</dt><dd>{String(data.status.runtime_log_entries ?? 0)} / {String(data.status.runtime_log_limit ?? data.limit)}</dd></div><div><dt>{t.adminRuntimeLogsRoutes}</dt><dd>{statValue(data.status.routes)}</dd></div><div><dt>{t.adminRuntimeLogsHostUptime}</dt><dd>{duration(data.status.host_uptime_seconds)}</dd></div></dl></Panel>
      <Panel id="load-title" title={t.adminRuntimeLogsHostLoad}><dl class="facts"><div><dt>{t.adminRuntimeLogsLoad}</dt><dd>{data.status.load_average?.join(" / ") || t.adminRuntimeLogsUnavailable}</dd></div><div><dt>{t.adminRuntimeLogsTotalMemory}</dt><dd>{bytes((data.status.host_memory?.total_kb || 0) * 1024)}</dd></div><div><dt>{t.adminRuntimeLogsAvailableMemory}</dt><dd>{bytes((data.status.host_memory?.available_kb || 0) * 1024)}</dd></div><div><dt>{t.adminRuntimeLogsCachedMemory}</dt><dd>{bytes((data.status.host_memory?.cached_kb || 0) * 1024)}</dd></div></dl></Panel>
    </div>
  {/if}
</section>

<style>
  .logs-page { display: grid; gap: 1rem; min-width: 0; } .panel-heading, .log-row { align-items: flex-start; display: flex; gap: .75rem; } .panel-heading { justify-content: space-between; }
  h2, p { overflow-wrap: anywhere; } h2 { margin: 0; font-size: 1.15rem; } .muted { color: #52606d; margin: .4rem 0 0; } .text-link { min-height: 2.5rem; padding: .55rem 0; }
  ...
  @media (max-width: 600px) { .panel-heading { align-items: stretch; flex-direction: column; } .status-grid { grid-template-columns: 1fr; } .log-row { display: grid; grid-template-columns: minmax(0, 1fr) auto; } .log-row time { grid-column: 1 / -1; } .log-row p { grid-column: 1 / -1; } }
</style>
