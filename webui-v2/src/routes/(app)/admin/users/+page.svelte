...
<section class="users-page" aria-labelledby="users-title">
  <header class="page-heading">
    <div>
      ...
      <h1 id="users-title">{t.adminUsersTitle}</h1>
      <p class="muted">{t.adminUsersDescription}</p>
    </div>
  ...
  <section class="panel create-panel" aria-labelledby="create-title">
    <details>
      <summary id="create-title">{t.adminUsersCreate}</summary>
  ...
  <section class="panel filters" aria-labelledby="filter-title">
    <h2 id="filter-title">{t.adminUsersFilters}</h2>
  ...
  {#if payload}
    <section class="list-panel" aria-labelledby="list-title">
      <div class="list-heading"><div><h2 id="list-title">{t.adminUsersList}</h2><p class="muted">{t.adminUsersPageOf.replace("{page}", String(payload.page)).replace("{pages}", String(Math.max(payload.pages, 1))).replace("{total}", payload.total.toLocaleString("zh-CN"))}</p></div></div>
...
<style>
  ...
  .page-heading, .heading-actions, .list-heading, .user-summary, .action-row, .pagination { align-items: flex-start; display: flex; gap: .75rem; }
  .page-heading, .list-heading, .user-summary, .pagination { justify-content: space-between; }
  ...
  h1, h2, h3, p { overflow-wrap: anywhere; } h1, h2, h3 { margin: 0; } h1 { font-size: 2.25rem; } h2 { font-size: 1.15rem; } h3 { font-size: .9rem; }
  ...
  .search-field { grid-column: span 2; } .list-panel { display: grid; gap: .9rem; } .user-list { display: grid; gap: .75rem; max-height: 70dvh; overflow: auto; overscroll-behavior: contain; scrollbar-color: #9fb3c8 #eef2f4; scrollbar-width: thin; }
  ...
  @media (max-width: 600px) { .page-heading, .heading-actions, .list-heading, .user-summary, .pagination { align-items: stretch; flex-direction: column; } .heading-actions .button, .heading-actions .text-link { width: 100%; } .create-form, .filter-form, .facts { grid-template-columns: 1fr; } .search-field { grid-column: auto; } .create-form .button, .filter-form .button { justify-self: stretch; width: 100%; } .badges { justify-content: flex-start; } .user-list { max-height: 68dvh; } .panel, .list-panel, .notice { padding: .85rem; } .pagination { align-items: stretch; text-align: center; } .pagination .button { width: 100%; } h1 { font-size: 1.65rem; } }
</style>
