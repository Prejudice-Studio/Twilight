...
<section class="emby-page" aria-labelledby="emby-title">
  <PageHeader id="emby-title" eyebrow={t.adminArea} title={t.adminEmbyTitle} description={t.adminEmbyDescription}>
    {#snippet actions()}
      <a class="text-link" href="/admin/status">{t.adminEmbyStatus}</a>
      <a class="button secondary" href={queryHref(data.tab)}>{t.adminEmbyRefresh}</a>
    {/snippet}
  </PageHeader>
  ...
  {#if data.tab === "accounts"}
     <Panel id="connectivity-title" className="connectivity-panel">
    ...
    <Panel id="users-title" className="users-panel">
...
<style>
  ...
  .panel-heading, .toolbar, .pagination, .activity-row, .audit-card header { align-items: flex-start; display: flex; gap: .75rem; }
  .panel-heading, .pagination, .audit-card header { justify-content: space-between; }
  .toolbar { align-items: center; flex-wrap: wrap; }
  h2, p { overflow-wrap: anywhere; } h2 { margin: 0; font-size: 1.2rem; }
  ...
  :global(.maintenance) { gap: .65rem; } .summary-grid { display: grid; gap: .65rem; grid-template-columns: repeat(4, minmax(0, 1fr)); } .summary-grid div { border: 1px solid #d7dee5; display: grid; gap: .2rem; padding: .7rem; } .summary-grid strong { font-size: 1.4rem; } .summary-grid span { color: #52606d; font-size: .8rem; } .tool-grid { display: grid; gap: .9rem; grid-template-columns: repeat(3, minmax(0, 1fr)); } .tool-form { border: 1px solid #d7dee5; display: grid; gap: .7rem; min-width: 0; padding: .85rem; } .tool-form h3 { font-size: 1rem; margin: 0; } .tool-form .button { justify-self: start; } .secret-result { display: grid; gap: .4rem; } .secret-result p { margin: 0; }
  ...
  @media (max-width: 600px) { .panel-heading, .toolbar, .audit-card header { align-items: stretch; flex-direction: column; } .panel-heading .button, .panel-heading form, .toolbar form, .toolbar .button { width: 100%; } .filters { grid-template-columns: 1fr; } .filters label:first-of-type { grid-column: auto; } .facts, .summary-grid { grid-template-columns: 1fr; } .test-row { grid-template-columns: 1.2rem minmax(0, 1fr); } .test-row span:last-of-type, .test-row small { grid-column: 2; } .tabs a { flex: 1 0 9rem; text-align: center; } .activity-row { flex-direction: column; } .activity-row time { max-width: none; text-align: left; } }
</style>
