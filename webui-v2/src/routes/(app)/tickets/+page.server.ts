...
import type { UserTicketDetailResponse, UserTicketListResponse } from "$lib/types";
...
export const load: PageServerLoad = async (event) => {
  ...
  const list = await apiJSON<UserTicketListResponse>(event, `/api/v2/tickets?page=${page}&per_page=20`, { cache: "no-store" });
  let detail: UserTicketDetailResponse | null = null;
  if (selectedID > 0) {
    const response = await apiJSON<UserTicketDetailResponse>(event, `/api/v2/tickets/${selectedID}`, { cache: "no-store" });
...
export const actions: Actions = {
  create: async (event) => {
    ...
    const result = await mutate(event, "/api/v2/tickets", "POST", {
      title,
      content,
  ...
  reply: async (event) => {
    ...
    const result = await mutate(event, `/api/v2/tickets/${id}/replies`, "POST", { content }, "reply", t.ticketReplyFailed);
    if (result instanceof Response) return result;
    if ("status" in result && typeof result.status === "number") return result;
  ...
  close: async (event) => {
    ...
    const result = await mutate(event, `/api/v2/tickets/${id}/close`, "POST", {}, "close", t.ticketCloseFailed);
    if (result instanceof Response) return result;
    if ("status" in result && typeof result.status === "number") return result;
    throw redirect(303, `/tickets?ticket=${id}`);
  },
  ...
  reopen: async (event) => {
    ...
    const result = await mutate(event, `/api/v2/tickets/${id}/reopen`, "POST", {}, "reopen", t.ticketReopenFailed);
    if (result instanceof Response) return result;
    if ("status" in result && typeof result.status === "number") return result;
    throw redirect(303, `/tickets?ticket=${id}`);
  },
  ...
  notify: async (event) => {
    ...
    const result = await mutate(event, `/api/v2/tickets/${id}/notify-telegram`, "PUT", { enabled }, "notify", t.ticketNotifyFailed);
    if (result instanceof Response) return result;
    if ("status" in result && typeof result.status === "number") return result;
    throw redirect(303, `/tickets?ticket=${id}`);
  }
