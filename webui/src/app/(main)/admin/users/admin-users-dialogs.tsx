...
import { AlertTriangle, CalendarClock, Eye, EyeOff, Link2, Loader2, Send, UserCheck, UserPlus } from "lucide-react";
...
import type { EmbyLibraryAccess, EmbyLibraryItem } from "@/lib/api-types";
...
/**
 * 媒体库权限对话框：调整某用户的 Emby 媒体库可见 / 隐藏 + 自助显隐授权。
 * - 数据 fetch / 变更 完全在父组件持有，对话框只负责呈现 + 触发 props 回调
 * - access 为空 / 未绑 Emby 时降级展示提示文案，不抛错
 */
export function LibraryAccessDialog({
  ...
  onOpenChange: (next: boolean) => void;
  target: UserInfo | null;
  access: EmbyLibraryAccess | null;
  selectedIds: Set<string>;
  isLoading: boolean;
  isActionLoading: boolean;
  onToggleSelected: (id: string) => void;
  onUpdateLibraries: (payload: Parameters<typeof api.updateUserLibraries>[1]) => void;
  onSetSelfService: (user: UserInfo, enabled: boolean) => void;
  onSave: () => void;
}) {
  return (
    <Dialog open={open} onOpenChange={(next) => { if (!isLoading) onOpenChange(next); }}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            {target ? `调整 ${target.username} 的 Emby 媒体库显示/隐藏权限` : "调整用户媒体库权限"}
          </DialogDescription>
        </DialogHeader>
        ...
        {isLoading && !access ? (
          ...
          <div className="space-y-4">
            <div className="flex flex-col gap-3 rounded-lg border bg-card/60 p-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center gap-2">
                <Badge variant={access.self_service_enabled ? "success" : "outline"}>
                  {access.self_service_enabled ? "已开启" : "未开启"}
                </Badge>
                {target && (
                  <Button
                    variant="outline"
                    ...
                    onClick={() => onSetSelfService(target, !access.self_service_enabled)}
                  >
                    {access.self_service_enabled ? "关闭授权" : "开启授权"}
                  </Button>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 rounded-lg border bg-card/60 p-3">
              <Badge variant={access.enable_all ? "default" : "secondary"}>
                {access.enable_all ? "策略：显示全部" : "策略：按选择显示"}
              </Badge>
              <Badge variant="outline">可见 {access.libraries.length}/{access.all_libraries.length}</Badge>
              {access.blocked_names.length > 0 && (
                <Badge variant="outline">隐藏：{access.blocked_names.join("、")}</Badge>
              )}
            </div>

            <div className="flex flex-wrap gap-2">
              <Button variant="outline" size="sm" disabled={isLoading} onClick={() => onUpdateLibraries({ action: "enable_all" })}>
                <Eye className="mr-2 h-4 w-4" />
                显示全部
              </Button>
              <Button variant="outline" size="sm" disabled={isLoading} onClick={() => onUpdateLibraries({ action: "disable_all" })}>
                <EyeOff className="mr-2 h-4 w-4" />
                隐藏全部
              </Button>
              {access.default_hidden_libraries.length > 0 && (
                <Button
                  variant="outline"
                  size="sm"
                  disabled={isLoading}
                  onClick={() => onUpdateLibraries({ action: "hide", library_names: access.default_hidden_libraries })}
                >
                  隐藏默认库
                </Button>
              )}
            ...
            <div className="space-y-2">
              ...
              <div className="grid max-h-72 gap-2 overflow-y-auto rounded-lg border p-3 sm:grid-cols-2">
                {access.all_libraries.map((lib) => {
                  const selected = selectedIds.has(lib.id);
                  return (
                    <button
                      key={lib.id}
                      type="button"
                      ...
                      onClick={() => onToggleSelected(lib.id)}
                    >
                      <div className="flex items-center justify-between gap-2">
            ...
            {access.self_service_libraries.length > 0 && (
              <div className="space-y-2 rounded-lg border p-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {access.self_service_libraries.map((name) => (
                    <div key={name} className="flex items-center gap-2 rounded-md bg-accent/50 px-2 py-1">
                      <span className="text-sm">{name}</span>
                      <Button variant="ghost" size="sm" disabled={isLoading} onClick={() => onUpdateLibraries({ action: "show", library_names: [name] })}>
                        显示
                      </Button>
                      <Button variant="ghost" size="sm" disabled={isLoading} onClick={() => onUpdateLibraries({ action: "hide", library_names: [name] })}>
                        隐藏
                      </Button>
                    </div>
                  ))}
        ...
        <DialogFooter>
          ...
          <Button
            onClick={onSave}
            disabled={isLoading || !access || !access.has_emby || access.all_libraries.length === 0}
          >
            {isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
            保存选择
          </Button>
...
/**
 * 批量设置媒体库显隐对话框。
 * - action 五档：show / hide / set / enable_all / disable_all
 * - 当 action 为 enable_all / disable_all 时不展示 library 列表
 */
export type BatchLibraryAction = "show" | "hide" | "enable_all" | "disable_all" | "set";

export function BatchLibraryDialog({
  open,
  onOpenChange,
  selectedUserCount,
  action,
  ...
  onActionChange: (next: BatchLibraryAction) => void;
    ...
    <Dialog open={open} onOpenChange={(next) => { if (!isLoading) onOpenChange(next); }}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            将对 {selectedUserCount} 个已选用户应用 Emby 媒体库显示策略。未绑定 Emby 的用户会自动跳过。
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div className="space-y-2">
            <Label>操作方式</Label>
            <Select value={action} onValueChange={(value) => onActionChange(value as BatchLibraryAction)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          {action !== "enable_all" && action !== "disable_all" && (
