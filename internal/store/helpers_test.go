...
func (s *Store) resetForTest(ctx context.Context) error {
	if _, err := s.db.ExecContext(ctx, `TRUNCATE twilight_runtime_logs, twilight_audit_logs, twilight_sessions, twilight_telegram_roster, twilight_telegram_runtime, twilight_playback_records, twilight_playback_events, twilight_playback_segments, twilight_playback_daily RESTART IDENTITY`); err != nil {
		return err
	}
