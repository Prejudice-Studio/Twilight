...
func developerJSDocs() map[string]any {
	examples := []map[string]string{
		...
		{
			"id":          "send-message",
			"title":       "Send message to any chat",
			"description": "Admin-only. Send a message to an arbitrary Telegram chat or user. Supports HTML parse_mode for formatted text.",
			"code":        "if (!admin.ensure()) return;\nconst chatId = Number(input.named('chat', 0));\nconst msg = input.text;\nif (!chatId || !msg) {\n  reply('Usage: /send --chat 123456789 Hello world');\n  return;\n}\nconst mid = sendMessage(chatId, msg, 'HTML');\nreply(mid ? 'Sent, message_id=' + mid : 'Failed to send');",
		},
		{
			"id":          "user-notify-prefs",
			"title":       "Update notification preferences",
			"description": "Update the current user's notification preferences including login and ticket Telegram notifications.",
			"code":        "const me = users.current();\nreply([\n  'login_tg=' + format.bool(me.notify_on_login_telegram, 'on', 'off'),\n  'ticket_tg=' + format.bool(me.notify_on_ticket_telegram, 'on', 'off'),\n  'login_email=' + format.bool(me.notify_on_login_email, 'on', 'off')\n].join('\\n'));",
		},
	}
	return map[string]any{
		...
		"functions": []developerJSDocEntry{
			...
			{Name: "sendMessage(chatId, text, parseMode)", Category: "network", Type: "function", Description: "Send a message to an arbitrary Telegram chat. Supports optional parse_mode (Markdown, MarkdownV2, HTML). Requires admin privileges. Returns the message_id on success, or false on failure.", Example: "const mid = sendMessage(123456789, 'Hello from sandbox', 'HTML');\nreply(mid ? 'sent: ' + mid : 'failed');", Params: []developerJSDocParam{jsDocParam("chatId", "number|string", "Target Telegram chat/user ID.", true), jsDocParam("text", "string", "Message text, truncated to 3900 characters.", true), jsDocParamDefault("parseMode", "string", "Optional parse mode: Markdown, MarkdownV2, HTML, or empty for plain text.", "")}, Returns: "number|boolean"},
		},
		"namespaces": []developerJSDocEntry{
			...
			{Name: "db.updateCurrentUser(patch)", Category: "db", Type: "function", Description: "Controlled write for the current user only. Accepted fields: notify_on_login_telegram / notify_on_ticket_telegram / notify_on_login_email, or telegram / email aliases.", Example: "const r = db.updateCurrentUser({ notify_on_ticket_telegram: true });\nreply(r.dry_run ? 'preview' : 'saved');", Params: []developerJSDocParam{jsDocParam("patch", "object", "Allowed booleans: notify_on_login_telegram, notify_on_ticket_telegram, notify_on_login_email, telegram, email.", true)}, Returns: "{ ok:boolean, dry_run?:boolean, user?:UserSnapshot, error?:string }", Mutates: true, Scope: "current_user_only"},
			{Name: "db.updateUser(uid, patch)", Category: "db", Type: "function", Description: "Alias of users.update(uid, patch). Admin-only controlled write with audit logging.", Example: "const r = db.updateUser(10001, { active: true });\nreply(r.ok ? 'saved' : r.error);", Params: []developerJSDocParam{jsDocParam("uid", "number|string", "Target Twilight UID.", true), jsDocParam("patch", "object", "Allowed fields: active, role, expired_at, notify_on_login_telegram, notify_on_ticket_telegram, notify_on_login_email, telegram, email.", true)}, Returns: "{ ok:boolean, dry_run?:boolean, user?:UserSnapshot, error?:string }", Mutates: true, Scope: "admin_only"},
			...
			{Name: "users.setLoginNotify(options)", Category: "users", Type: "function", Description: "Update the current bound user's notification preferences. Accepted booleans: telegram (login), ticket_tg (ticket), email (login).", Example: "const r = users.setLoginNotify({ ticket_tg: true });\nreply(r.ok ? 'saved' : r.error);", Params: []developerJSDocParam{jsDocParam("options", "object", "Allowed booleans: telegram, ticket_tg, email.", true)}, Returns: "{ ok:boolean, dry_run?:boolean, user?:UserSnapshot, error?:string }", Mutates: true, Scope: "current_user_only"},
			...
			{Name: "users.update(uid, patch)", Category: "users", Type: "function", Description: "Admin-only combined controlled update. Accepted patch fields: active, role, expired_at, notify_on_login_telegram / notify_on_ticket_telegram / notify_on_login_email, telegram / email aliases.", Example: "const r = users.update(10001, { active: true, notify_on_ticket_telegram: true });\nreply(r.ok ? 'saved' : r.error);", Params: []developerJSDocParam{jsDocParam("uid", "number|string", "Target Twilight UID.", true), jsDocParam("patch", "object", "Allowed fields: active, role, expired_at, notify_on_login_telegram, notify_on_ticket_telegram, notify_on_login_email, telegram, email.", true)}, Returns: "{ ok:boolean, dry_run?:boolean, user?:UserSnapshot, error?:string }", Mutates: true, Scope: "admin_only"},
			...
			{Name: "admin.updateUser(uid, patch)", Category: "admin", Type: "function", Description: "Shortcut for users.update(uid, patch). Admin-only controlled write with audit logging.", Example: "const uid = Number(input.named('uid', 0));\nconst r = admin.updateUser(uid, { notify_on_ticket_telegram: true });\nreply(r.ok ? 'saved' : r.error);", Params: []developerJSDocParam{jsDocParam("uid", "number|string", "Target Twilight UID.", true), jsDocParam("patch", "object", "Allowed user patch fields: active, role, expired_at, notify_on_login_telegram, notify_on_ticket_telegram, notify_on_login_email.", true)}, Returns: "{ ok:boolean, dry_run?:boolean, user?:UserSnapshot, error?:string }", Mutates: true, Scope: "admin_only"},
		...
		"config_keys": []string{
			...
			"telegram.enabled", "global.telegram_mode", "telegram.force_bind", "global.force_bind_telegram", "telegram.require_membership", "telegram.panel_enabled", "telegram.ban_on_leave", "telegram.parse_mode",
			"invite.enabled", "invite.max_depth", "invite.limit", "invite.root_user_limit",
			"email.enabled", "email.force_bind", "media_request.enabled", "signin.enabled", "ticket.enabled", "ticket.notify_telegram_template", "limits.user", "limits.emby_user",
		},
		"env_keys": []string{
