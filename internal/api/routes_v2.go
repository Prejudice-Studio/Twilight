...
func (a *App) registerV2Routes() {
	...
	// 已删除 /api/v2/admin/users/:uid/registration-entitlement/dequeue：与上一行共用
	// handler、参数与返回完全一致，"出队"从未实现，且后端不存在运行时注册队列。
	// 需要"移出等待队列"时请显式调用 /admin/users/{uid}/registration-queue/clear。
	...
	// 注意：/api/v2/admin/telegram/roster/stats 已在上面的 admin 分组注册过
	...
	// 注意：这里必须是按 id 删除的 handleV2DeleteMediaRequest。曾经错挂成
