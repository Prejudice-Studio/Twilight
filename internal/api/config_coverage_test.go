...
func TestConfigSchemaSurfacesEveryConfigField(t *testing.T) {
	// config.Load 会把 TWILIGHT_* 环境变量当作最高优先级覆盖。若测试进程里带着
	// 这类变量（例如本地跑真库测试时设了 TWILIGHT_DATABASE_URL），被覆盖的字段
	// 读回来就是环境变量的值而不是本用例塞进去的特征值，会被误报成"没纳管"。
	// 这里在用例期间清空并在结束后还原。
	clearTwilightEnvForTest(t)
...
// clearTwilightEnvForTest 在用例期间移除所有 TWILIGHT_* 环境变量，结束后还原。
func clearTwilightEnvForTest(t *testing.T) {
	t.Helper()
	for _, entry := range os.Environ() {
		...
		if !strings.HasPrefix(strings.ToUpper(key), "TWILIGHT_") {
			continue
		}
