...
var (
	...
	placeholderHintsGroupPanel = []string{
		...
		"{expire_status}", "{expired_at}", "{register_time}", "{created_at}",
		"{telegram_status}", "{telegram_username}", "{telegram_userid}",
		"{emby_status}", "{emby_bound_status}", "{emby_bound}", "{emby_enabled_status}",
		"{emby_username}", "{emby_unbind_allowed}",
		"{registration_source}", "{registration_code}", "{pending_emby}", "{pending_emby_days}",
		"{emby_remote_block}", "{emby_remote_status}", "{emby_remote_username}",
		"{emby_remote_enabled}", "{emby_remote_role}", "{emby_remote_hidden}", "{emby_last_activity}",
...
func configSectionDefs() []configSectionDef {
	...
	return []configSectionDef{
		...
		{Key: "Emby", Title: "Emby", Description: "Emby 连接和线路", Category: "integration", Collapsed: true, Fields: []configFieldDef{
			...
			{Key: "play_rank_user_visible", Label: "排行榜对普通用户开放", Type: "bool", Description: "登录的普通用户可在侧边栏入口查看脱敏后的排行榜（用户名打码）"},
			{Key: "play_rank_anonymous", Label: "排行榜对无账号访客开放", Type: "bool", Description: "未登录访客可读取脱敏榜单；前端暂无公开入口，开启后仅对直连接口生效"},
		}},
		{Key: "Telegram", Title: "Telegram", Description: "Bot、订阅校验和群组管理\n推荐在 Telegram 管理页面操作 Bot 基础设置，高级参数在此调整", Category: "integration", Collapsed: true, Fields: []configFieldDef{
			...
			{Key: "bot_start_intro", Label: "开始简介", Type: "textarea", Description: "内置 /start 文案的简介，支持换行", PlaceholderHints: placeholderHintsBotText},
			{Key: "bot_bind_prompt_text", Label: "绑定提示文案", Type: "textarea", Description: "/bind 无参数时的提示文本，支持换行", PlaceholderHints: placeholderHintsBotText},
			{Key: "bot_help_text", Label: "用户帮助文案", Type: "textarea", Description: "覆盖 /help 和 /twihelp 文案，支持换行", PlaceholderHints: placeholderHintsBotText},
			{Key: "bot_admin_help_text", Label: "管理员帮助文案", Type: "textarea", Description: "覆盖 /twishelp 文案，支持换行", PlaceholderHints: placeholderHintsBotText},
			{Key: "bot_help_header", Label: "帮助页前缀", Type: "textarea", Description: "追加到内置用户帮助顶部，支持换行", PlaceholderHints: placeholderHintsBotText},
			{Key: "bot_help_footer", Label: "帮助页后缀", Type: "textarea", Description: "追加到内置用户帮助底部，支持换行", PlaceholderHints: placeholderHintsBotText},
			{Key: "bot_about", Label: "Bot 关于文案", Type: "textarea", Description: "/about 的服务说明，支持换行", PlaceholderHints: placeholderHintsBotText},
		...
		{Key: "SAR", Title: "注册/邀请", Description: "注册、卡码、邀请关系和求片\n推荐在「注册码管理」和「邀请系统管理」页面操作", Category: "policy", Collapsed: true, Fields: []configFieldDef{
			...
			{Key: "signin_auto_renewal_enabled", Label: "允许自动积分续期", Type: "bool", Description: "允许已主动开启个人开关的用户在到期检查时自动扣除签到积分续期"},
		...
		{Key: "Notification", Title: "通知", Description: "用户通知策略与登录通知模板", Category: "policy", Collapsed: true, Fields: []configFieldDef{
			...
			{Key: "login_notify_telegram_template", Label: "登录通知 TG 模板", Type: "textarea", Description: "Telegram 通知模板。占位符：{username}、{time}、{ip}、{device}；留空使用内置默认；支持换行", PlaceholderHints: placeholderHintsLoginNotify},
			{Key: "login_notify_email_subject_template", Label: "登录通知邮件标题", Type: "string", Description: "邮件通知标题。占位符：{server_name}、{username}、{time}、{ip}、{device}", PlaceholderHints: placeholderHintsLoginNotify},
			{Key: "login_notify_email_body_template", Label: "登录通知邮件正文", Type: "textarea", Description: "邮件通知正文。占位符：{username}、{time}、{ip}、{device}；留空使用内置默认；支持换行", PlaceholderHints: placeholderHintsLoginNotify},
		}},
		...
		{Key: "Ticket", Title: "工单系统", Description: "用户提交工单与管理员处理；工单类型请在「工单处理」页面管理", Category: "policy", Collapsed: true, Fields: []configFieldDef{
			...
			{Key: "notify_telegram_template", Label: "TG 通知模板", Type: "textarea", Description: "工单变动通知模板。占位符：{ticket_id}、{title}、{status}、{priority}、{type}、{admin_note}、{time}、{server_name}；留空使用内置默认；支持换行", PlaceholderHints: placeholderHintsTicketNotify},
		}},
		...
		{Key: "Email", Title: "邮箱验证", Description: "SMTP 发信、验证码与强制绑定策略\n推荐在「邮箱管理」页面操作基础发信配置，高级参数在此调整", Category: "integration", Collapsed: true, Fields: []configFieldDef{
			...
			{Key: "subject_template", Label: "邮件标题模板", Type: "string", Description: "占位符：{site} 站点名、{code} 验证码、{ttl} 有效分钟数", PlaceholderHints: placeholderHintsEmailCode},
			{Key: "body_template", Label: "邮件正文模板", Type: "textarea", Description: "占位符：{site} 站点名、{code} 验证码、{ttl} 有效分钟数；支持换行", PlaceholderHints: placeholderHintsEmailCode},
...
func configValues(cfg config.Config) map[string]map[string]any {
	return map[string]map[string]any{
		...
		"Emby": {
			...
			"play_rank_anonymous": cfg.PlayRankAnonymous,
		},
		"Telegram": {
		...
		"SAR": {
			...
			"reset_after_miss": cfg.SigninResetAfterMiss, "signin_renewal_enabled": cfg.SigninRenewalEnabled, "signin_auto_renewal_enabled": cfg.SigninAutoRenewalEnabled, "signin_renewal_cost": cfg.SigninRenewalCost, "signin_renewal_days": cfg.SigninRenewalDays,
		},
