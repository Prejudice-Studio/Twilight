...
func TestV2UserAnnouncementResourcesRequireUserAndDoNotCache(t *testing.T) {
	...
	// 公开列表：匿名可读，但**不得**带任何按用户计算的状态。
	public := doJSON(app, http.MethodGet, "/api/v2/announcements", "", nil)
	if public.Code != http.StatusOK {
		t.Fatalf("public announcement list should be readable anonymously, got %d body=%s", public.Code, public.Body.String())
	}
	if strings.Contains(public.Body.String(), "unseen_force_read") {
		t.Fatalf("public announcement list must not carry per-user force-read state: %s", public.Body.String())
	}

	// 个性化列表：必须登录。
	unauthenticated := doJSON(app, http.MethodGet, "/api/v2/me/announcements", "", nil)
	if unauthenticated.Code != http.StatusUnauthorized {
		t.Fatalf("expected unauthenticated personalised announcement read to fail, got %d body=%s", unauthenticated.Code, unauthenticated.Body.String())
	}
