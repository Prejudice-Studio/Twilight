...
- Media detail desktop layout must keep the poster at its natural source aspect ratio and full width in a poster-only left column; title, rating, source, original title and Logo belong in the right-column header. Do not use a fixed poster aspect-ratio wrapper, scale the poster down to fit a black stage, or use an absolute bottom overlay for the primary title. The dialog content must have an explicit viewport-constrained height and independent detail-column scrolling so tall metadata cannot clip the poster or action bar.
