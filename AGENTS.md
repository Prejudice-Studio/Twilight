...
- The media detail dialog derives its desktop width and height from the poster's intrinsic dimensions after image load, clamps the poster as a whole to the viewport, and applies the same computed height to the right detail column. Keep the poster fully visible with `object-contain`; do not let a bottom-aligned image overflow above a clipped parent.
