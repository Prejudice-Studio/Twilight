...
- Never format UI strings on the backend. The episode badge is rendered by `webui/src/components/play-rank-media-label.tsx` from `playRank.seasonEpisode` (`S{season}E{episode}`) and `playRank.episodeOnly` (Chinese: 第 {n} 集). Only-episode rows must NOT degrade to a made-up `E8` — that hides "we don't know the season" behind an invented abbreviation, and it is untranslatable.
