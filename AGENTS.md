...
- User password-security preferences are backend-enforced. System password changes always verify the current Web password; optional email-code gates for system/Emby password changes and the optional current-Web-password gate for Emby password changes must be enforced in handlers. Emby self-service password changes use one proof method at a time: enabling the current Web-password proof disables the personal Emby email-code proof, and legacy records with both enabled give precedence to the Web password. Disabling personal email-code protection is allowed without an email code only for the currently authenticated administrator; ordinary users must provide the matching email proof. Disabling the Web-password protection still requires the current Web password. Security preference updates must accept only JSON boolean values; frontend validation is UX only and must not be the security boundary.
