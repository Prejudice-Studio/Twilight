...
- The V2 frontend rewrite is enforced in `webui-v2/scripts/check-architecture.mjs`: keep the root layout explicitly SSR-enabled and non-prerendered, keep production on `@sveltejs/adapter-node`, keep legacy paths as server redirects only, and do not reintroduce React/Next/Zustand, direct browser API calls, SSE/WebSocket, or client polling. The old `webui/` tree is rollback-only and must not become a new feature target.
