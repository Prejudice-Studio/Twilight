...
- Admin invite quick maintenance lives at `POST /admin/invite/quick-maintenance`. It requires `INVITE_QUICK_MAINTENANCE`, supports `scope=selected|subtree|all`, may detach relations and renew affected child users, must protect admin accounts from renewal changes, and must remain available when the invite system is disabled.
