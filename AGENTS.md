...
- Media detail desktop layout must keep the poster at its natural source aspect ratio and full width in a balanced, bottom-aligned poster-only left column; when a Logo exists, render it as transparent unframed artwork on the left of the right-column header and place title/original title metadata to its right. Do not use a fixed poster aspect-ratio wrapper, scale the poster down to fit a black stage, add a Logo border/background, or use an absolute bottom overlay for the primary title. The dialog content must have an explicit viewport-constrained height and independent detail-column scrolling so tall metadata cannot clip the poster or action bar.
- The media detail dialog derives its desktop height from the available viewport and derives only the left column width from the poster's intrinsic aspect ratio. The right detail column keeps a usable minimum width and scrolls independently. Keep the poster fully visible with `object-contain`; do not let a bottom-aligned image overflow above a clipped parent or let image dimensions force the whole dialog below the usable viewport width.
