...
- The V2 admin Bangumi route is SSR and manual-action only. User status is server-paginated, playback records and sync logs are fetched only for the selected UID, and per-page record/sync counts must use bounded batch reads. Bangumi tokens never enter the response; Go remains responsible for feature gates, user existence, external sync, persistence, and audit.
