...
- User password-security preferences are backend-enforced. System password changes always verify the current Web password; optional email-code gates for system/Emby password changes and the optional current-Web-password gate for Emby password changes must be enforced in handlers. Disabling those user-level protections must require the matching proof (email code or current Web password), not only a frontend switch.
