...
- Feature switches must gate the page, not just the navigation entry. `filterNavItems` hides sidebar items whose `features` flag is off (`media_request`, `signin`, `invite`, `play_rank`/`play_rank_user`, `bangumi_sync`+`bangumi_manage`, `ticket_system`, `developer_mode`), but a hidden link is not access control: users still arrive through bookmarks, history, or a typed URL. Every user-facing page behind a switch must also render `FeatureDisabledNotice` (via `useFeatureEnabled`) before its first data read, so a disabled feature shows one clear conclusion instead of a page of 403s and empty lists. Keep the page's flag semantics identical to the sidebar's — Bangumi hides only when both `bangumi_sync` and `bangumi_manage` are off.
- The gate wrapper must be the exported component and the body its inner component (`export default function X() { if (!enabled) return <FeatureDisabledNotice/>; return <XContent/>; }`). Do not put the early return after the page's own hooks — that still fires the disabled feature's requests on every visit.
- On the admin ticket detail page, status / priority / type save **immediately** through a single-field patch (`patchTicket`), with no local draft. Only the internal `admin_note` keeps a draft plus an explicit save button: it is long text, and autosaving every keystroke is worse than one deliberate click. Never reintroduce a shared "save metadata" button over status drafts — replying auto-transitions `open → in_progress`, and a stale draft then silently reverted it back to `open`.
- The admin ticket detail page must keep the two text surfaces visually unmistakable: the left reply box is labelled "visible to the user", the right internal note is labelled "admins only" with its own muted background and a save button. Confusing them sends internal notes to users, or replies nowhere.
- Destructive controls on admin pages (ticket deletion) belong to their own bordered danger zone at the end of the sidebar, not inline among read-only metadata where they read as just another action.
- Never derive a label from a parameterized i18n string by stripping the placeholder (`t("tickets.createdAt", { time: "" }).replace(...)`). Add a dedicated label key instead — the strip trick silently breaks for any locale whose phrasing puts the placeholder mid-sentence.
- Filter toolbars must not rely on a long `xl:grid-cols-[…]` list of fixed-width tracks. Its minimum widths add up past the available width on common laptop viewports and push the trailing buttons out of the container. Split filters and actions into rows with plain responsive column counts.
...
- Aggregation lives in `store.PlaybackRank(PlaybackRankOptions{Since, Limit, GroupBy, SortBy})`: two `GROUP BY` queries with a `played_at >= $1` filter, falling back to an in-memory scan of `state.PlaybackRecords` when PostgreSQL is unavailable. `duration` is **seconds**. The options struct (not positional args) exists because two of the four params are strings and two are ints — swapping `since` and `limit` compiles fine and silently returns wrong data.
- `sort_by` is `plays` (default) or `duration` and applies to **both** boards. They answer different questions — "how often was it started" vs "how long was it watched" — and a 20-min episode replayed 30 times outranks a 3-hour movie on plays but loses on duration. Never pick one for the caller. Historically the media board was hardcoded to plays and the user board to duration, so the two "first places" were not comparable; both now follow `sort_by`.
- Sort expressions come only from the whitelisted `playbackRankOrderBy`; never let an external string reach `ORDER BY`. The secondary key is always the other metric (stable on ties — otherwise ranks jump between refreshes) and a unique key (`item_id` / column ordinal / `uid`) breaks the rest. The in-memory comparator `playbackRankLess` mirrors it exactly: **rankings must not change when PostgreSQL degrades**.
- The media board has two grouping modes: `item` (one row per episode/movie, default) and `series` (all episodes of a show merged). `series` groups by `COALESCE(NULLIF(series_name,''), NULLIF(title,''), item_id)` because no SeriesId is stored — **same-named shows collapse into one row**, a known approximation. Unknown `group_by` values must fall back to `item`; never pass them through to `GROUP BY`.
- `episodes` on a media row is how many distinct items it covers (always 1 in `item` mode). Without it you cannot tell "30 people watched one episode each" from "one person rewound the same episode 30 times".
- Season/episode numbers are **not** stored. `playRankEpisodes` resolves `ParentIndexNumber`/`IndexNumber` from Emby in bulk while building the board and emits `season_number` / `episode_number` as **numbers only**. Historical rows show episode codes with no migration; when Emby is unreachable the numbers simply go missing and the board still returns. No episode number means neither field is emitted.
- Never format UI strings on the backend. The episode badge is rendered by `webui/src/components/play-rank-media-label.tsx` from `playRank.seasonEpisode` (`S{season}E{episode}`), `playRank.episodeOnly` (Chinese: 第 {n} 集) and `playRank.episodeUnknown`. Two rules follow from this: (a) only-episode rows must NOT degrade to a made-up `E8` — that hides "we don't know the season" behind an invented abbreviation, and it is untranslatable; (b) when a row *should* have episode numbers but we couldn't read them (Emby unreachable, item deleted from the library, metadata never scraped), render a visually weakened "未知" placeholder instead of leaving it blank or defaulting to `S1E8`. A series-name-only row reads as "the whole show" when it is actually one episode; a fabricated code is worse than a blank because it looks like real data.
- Windows are computed server-side in `playRankWindow`: `day` = today 00:00, `week` = Monday 00:00, `month` = the 1st 00:00, `all` = `0` (no lower bound, i.e. everything the system has recorded). Server local time for all calendars. Do not move window math into the frontend.
- `days=N` is a trailing-N-days sliding window that overrides `range` when present, clamped to `playRankMaxDays` (730). The effective key echoes back as `30d` and becomes part of the cache key.
- `store.PlaybackRecordCoverage()` returns the whole-table extent (`total`, `earliest`, `latest`) and is surfaced as `recorded`. It is deliberately **not** window-scoped: when the current window is empty but `recorded.total > 0`, the UI must hint at switching to `all` instead of showing a bare "no data". A window with no rows must never be read as "the system recorded nothing".
- Playback Reporting matching is deliberately tolerant (12h slack, nearest candidate) because the plugin's timestamp convention is not knowable — it may record start or stop, and its SQLite timestamps may be UTC while activity logs use server local time. Do not "tighten" this window without a way to determine the plugin's actual convention; a miss costs a duplicated play, which is far worse than two sessions swapping durations.
...
- Cache: 60s per `range|since|limit|identity`, `refresh=1` bypasses it, and `invalidatePlayRankCache` runs after activity-log sync persists new records.
- Two data sources, two duration definitions. Activity logs (built-in, no plugin) pair playback start/stop events, so the duration is a wall-clock gap and pauses count. The Playback Reporting plugin — the one named "Playback Reporting" in the Emby plugin catalog, whose API prefix `/emby/user_usage_stats/...` is often mislabeled "UserUsageStats" — records `PlayDuration - PauseDuration`, i.e. net watched time. Both are called "play duration" and they are not the same number; never swap one for the other silently.
- Playback Reporting is an enhancement, never a hard dependency: `syncPlaybackReporting` swallows every error and returns zeros so a missing plugin can never break the activity-log sync or the scheduler job. Probe results are cached for 10 minutes (`playbackReportingReady`) — do not probe per request, and do not log a warning per sync when the plugin is simply absent.
- `store.ApplyPlaybackReportingRecords` must correct, never duplicate: it looks for the same `(uid, item_id)` row that is closest in time and not already corrected, rewrites its `duration` and sets `source = playback_reporting`; it inserts only when the activity-log source has no row for that session. If it ever inserts blindly, every play is counted twice and the ranking doubles.
- `twilight_playback_records.source` (`activity_log` / `playback_reporting`) exists so a row's duration can be interpreted. Backfilled rows are `activity_log`. Any new read path that reports durations should surface which source it came from.
...
- SSRF defence has two layers and both must stay. URL-layer validation (`validateOutboundBaseURL`) only sees the configured string, so it can only judge literal IPs; `guardOutboundDialAddress` on the dialer's `Control` hook sees the **resolved** IP and is what actually covers hostnames and DNS rebinding. Do not "simplify" by deleting one of them.
- Loopback is deliberately allowed on outbound calls (same-host docker-compose deployments and `httptest` both depend on it) and RFC1918 is deliberately not blocked (self-hosted Emby typically lives on a private address). Do not tighten either without a deployment story.

## Request Parameter Rules

- Parse query parameters through `internal/api/params.go` (`queryInt` / `queryInt64` / `queryIntClamped`). Do not write inline `strconv.Parse*` with a discarded error: `limit=abc` silently becoming `0` is only safe in handlers where `0` happens to mean "unset", and reading the handler is the only way to know which it is.
- Optional presentation parameters (`limit`, `since`, filters) fall back to the caller's explicit default on parse failure. They must **not** start returning 400 — that breaks existing clients. Required path parameters keep using `int64Param`, which returns an error so the caller can decide.
- Bounds belong at the call site as visible numbers (`queryIntClamped(r, "limit", 100, 1, 1000)`), not buried in `if parsed > 0 && parsed <= 1000`.

## Go Formatting Rules

- Run `gofmt -w` on files you touched only. Many files in this repo are CRLF, so `gofmt -l internal/api` reports ~100 "unformatted" files that are just line-ending differences; batch-formatting the package rewrites files you never meant to touch and buries your change in noise.
