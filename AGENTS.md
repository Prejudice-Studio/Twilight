...
- Media detail desktop layout must keep the poster in a normal-flow aspect-ratio container with title metadata below it; do not use an absolute bottom overlay for the primary title. The dialog content must have an explicit viewport-constrained height and independent detail-column scrolling so tall metadata cannot clip the poster or action bar. Use `object-contain` for posters to avoid cropping source artwork.
