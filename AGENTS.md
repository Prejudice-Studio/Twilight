...
- Season/episode numbers are **not** stored. `playRankEpisodeLabels` resolves `ParentIndexNumber`/`IndexNumber` from Emby in bulk while building the board and emits `episode_label` (`S1E8`). This is why historical rows show episode codes with no migration; when Emby is unreachable the label simply goes missing and the board still returns. Only-episode degrades to `E8`; no episode number means no label at all.
