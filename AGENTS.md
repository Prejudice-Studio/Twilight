...
- The V2 administrator email page uses shared `PageHeader.svelte` and `Panel.svelte` structure. Keep the pending/account lists server-paginated, preserve generic SMTP errors and maintenance confirmation phrases, and use the shared panel root only for layout rather than client-side email state.
