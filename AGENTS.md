...
- Shared `Input` and `SelectTrigger` use a 44px mobile / 40px desktop baseline, while ordinary `Button` keeps the same minimum height; do not introduce per-page default control heights that make one toolbar drift. The shared `Card` baseline is an 8px radius unless a page explicitly needs a different surface.
