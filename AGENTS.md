...
- Aggregation lives in `store.PlaybackRank(since, limit, groupBy)`: two `GROUP BY` queries with a `played_at >= $1` filter, falling back to an in-memory scan of `state.PlaybackRecords` when PostgreSQL is unavailable. `duration` is **seconds**.
