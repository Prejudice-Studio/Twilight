...
- `webui-v2/src/routes/(app)/admin/database`: Admin-only SSR database status, backup maintenance, restore preview, and guarded legacy database migration actions.
- `webui-v2/src/routes/(app)/admin/migration`: Admin-only SSR Twilight archive export/import preview and confirmed migration actions.
...
- In V2, the shared control baseline is implemented by `webui-v2/src/lib/app.css` and the application shell consumes its semantic low-saturation blue-gray tokens. New route-local controls must inherit those constraints or explicitly document a stable format exception; do not introduce a second global palette, purple/indigo action fills, or fixed-width controls that bypass the viewport boundary.
- V2 page structure should use the stateless shared `PageHeader.svelte` and `Panel.svelte` primitives where a route needs a page heading or framed section. They own responsive wrapping and semantic headings; route CSS should style only domain-specific content and must not recreate a second global header/panel baseline.
- The V2 administrator users page follows the shared structure baseline: `PageHeader.svelte` owns the page title/actions, and `Panel.svelte` owns create/filter/list sections. Keep server-side pagination, grouped account/Emby/identity/danger actions, bounded list scrolling, and SvelteKit form actions unchanged when adjusting its layout.
...
- The V2 frontend rewrite is enforced in `webui-v2/scripts/check-architecture.mjs`: keep the root layout explicitly SSR-enabled and non-prerendered, keep production on `@sveltejs/adapter-node`, require every migrated legacy page to have a real `+page.svelte` (or an explicitly listed server redirect), require every non-static page to have a `+page.server.ts` boundary, and do not reintroduce React/Next/Zustand, direct browser API calls, SSE/WebSocket, or client polling. The old `webui/` tree is rollback-only and must not become a new feature target.
- The V2 server status page must read `/api/v2/admin/health/api`, `/api/v2/admin/health/database`, and `/api/v2/admin/health/emby` as three separate private no-store resources. `/api/v2/admin/stats` is separate from all probes, and `/api/v2/system/info` is a safe public projection for the shell/setup pages. The page performs these independent reads from a server `load` with `Promise.allSettled`, then returns only safe display DTOs to the browser; it has no client polling. An unavailable probe must not discard successful results from the other probes, and a non-admin must be rejected by the server-side admin layout rather than hidden navigation.
...
- The V2 administrator scheduler page uses shared `PageHeader.svelte` and `Panel.svelte` structure for the manual refresh entry, filter, empty state, and run detail. Keep job cards domain-specific, preserve server-side parameter rebuilding and form actions, and do not add browser-side task state or automatic refresh.
...
- The V2 `/admin/audit-logs` page must use `/api/v2/admin/audit-logs` for server-side audit history: filters and sort values are normalized into the URL, only the bounded backend page is sent to the browser, and all delete/clear/prune mutations use SvelteKit form actions with server-side confirmation phrases. Audit maintenance must not append a recursive record to the same table after deletion, clearing, or pruning. Audit detail is rendered as escaped text, destructive controls require confirmation, and the log list owns a bounded Firefox scroll region.
- The V2 `/admin/violations` page must keep violation history server-side: type/search/page values are normalized into the URL, only the bounded backend page is sent to the browser, and delete/clear mutations use SvelteKit form actions against `/api/v2/admin/violations` with server-side ID/confirmation validation. Violation fields are rendered as escaped text and the list owns a bounded Firefox scroll region.
- The V2 administrator violations page uses shared `PageHeader.svelte` and `Panel.svelte` structure. Keep filtering, destructive confirmation, escaped evidence rendering, and bounded list scrolling in the route; shared components must not become a second audit or authorization layer.
- The V2 administrator email page uses shared `PageHeader.svelte` and `Panel.svelte` structure. Keep the pending/account lists server-paginated through `/api/v2/admin/email/verifications`, preserve generic SMTP errors and maintenance confirmation phrases, and use the shared panel root only for layout rather than client-side email state.
- The V2 Telegram rebind review must use `/api/v2/admin/telegram/rebind-requests` for server-filtered pagination and all mutations. The V2 adapter must retain the canonical request-state transitions, batch bounds, and audit writes from the shared handlers; no state can be inferred or approved in SvelteKit.
- The V2 administrator announcements page uses shared `PageHeader.svelte` and `Panel.svelte` for the operational title, filter, and list regions. Keep the native disclosure editor independent, preserve pagination and escaped content rendering, and leave field validation, render-mode policy, revision behavior, and audit to Go.
...
- The V2 administrator registration-code page uses shared `PageHeader.svelte` and `Panel.svelte` structure. Keep generated-code, filter, list, and usage panels server-rendered; do not move code values or usage history into a browser cache, and use `:global` only when a route intentionally styles a shared panel root.
...
- The V2 administrator invite page uses shared `PageHeader.svelte` and `Panel.svelte` structure for its SSR title and tree/code/config/detail sections. Keep custom tree/table and dangerous-action styles scoped to the invite domain; use `:global` only for intentional styling of a shared panel root.
- The V2 `/admin/bangumi` page must use the paginated admin users endpoint for the first view, keep playback records and sync logs out of the list payload, and load one selected user's bounded detail through URL state. Sync and log-clearing operations use SvelteKit form actions; no browser polling or full-user client cache is allowed.
...
- The V2 administrator media-request page uses the shared `PageHeader.svelte` and `Panel.svelte` structure. Do not restore route-local global heading/panel CSS while changing its request cards, source badges, grouped actions, or narrow-screen controls.
...
- The V2 administrator runtime-log page uses shared `PageHeader.svelte` and `Panel.svelte` structure. Keep the runtime summary, bounded stream, and operational facts separately readable; do not restore persistent log state, SSE, polling, or raw diagnostics in browser code.
...
- The V2 `/admin/telegram` page must load `/api/v2/admin/config/schema`, `/api/v2/admin/telegram/commands/catalog`, and `/api/v2/admin/telegram/roster/stats` on the server with `Promise.allSettled`; only an allowlisted non-secret Telegram subset may reach the browser. Configuration changes use a SvelteKit form action and the Go schema endpoint remains the final validation, reload, and audit boundary. Bot tests use `/api/v2/admin/telegram/test`, are manual only, return a safe summary without upstream errors, and do not use polling or SSE.
...
- The V2 user media-request page is server-rendered at `webui-v2/src/routes/(app)/media`. Search, selected detail, and inventory reads use `/api/v2/media/*` through URL-driven server loads; create/delete mutations use SvelteKit form actions. Do not move business permission checks or inventory rules into client JavaScript, and keep the V1 page available as the migration fallback.
...
- V2 migrated modules must use server `load` for session-scoped read aggregation and SvelteKit form actions for writes. The migrated settings page is the reference implementation: `/api/v2/settings` is the single SSR read, `/api/v2/settings/preferences` uses `PUT`, and password, email-verification, binding, and preference decisions must not move into browser-only state.
...
- The V2 appearance page reads one private `GET /api/v2/settings/appearance` projection and writes only through `/api/v2/settings/appearance/background*` and `/api/v2/settings/appearance/avatar*`. Keep the legacy `/api/v1/users/assets/{kind}/{filename}` resource URL for protected image delivery until a separately reviewed asset-resource migration exists; do not add a second asset authorization implementation.
- The V2 `/settings/apikey` page must read only the current user's masked API Key summaries from `/api/v2/settings/apikeys` with SSR `load`; creation, update, and deletion must use form actions against the same resource family. A plaintext Key may appear only in the current creation response and must never be put in a URL, browser cache, persistent page state, or logs. Name, rate-limit, identifier, and boolean parsing at the action boundary are UX/resource guards only; Go remains the final ownership, permission, audit, and persistence boundary.
- The V2 `/admin/developer` page must load the developer-mode-sensitive preset list and sandbox documentation on the server with no-store reads. Preview and preset mutations use form actions; browser checks only bound input resources and must not replace the Go sandbox validator, timeout, output cap, developer-mode gate, or audit. Script output, log output, documentation, and preset text are untrusted display data and must remain text-rendered inside bounded Firefox scroll regions.
- The V2 administrator user page uses the native `/api/v2/admin/users` resource collection with `items`/`pagination`; it must render only the current page, keep account/Emby/identity/dangerous actions grouped, and send every mutation through a server form action. The V2 resource reuses the audited V1 Store/handler rules during migration, while the page itself must not call `/api/v1/admin/users`. The `admin_action_state` field is display guidance only; Go handlers remain the final authorization boundary.
...
- V2 authentication resources are the runtime boundary for `/login`, `/register`, `/forgot-password`, `/logout`, and the root session lookup. Use `/api/v2/auth/*` and `/api/v2/registration/*` from SSR routes; do not expose tokens or passwords in page data, URLs, or browser storage. Public capability flags must describe only booleans and must not embed configuration secrets or upstream diagnostics. Registration bind-code creation uses POST plus the existing WebUI intent headers so the V2 resource does not add a side-effecting GET.
...
- The V2 admin ticket route is the first native V2 resource migration: `webui-v2` must use `/api/v2/admin/tickets` with `items`/`pagination`, `/api/v2/admin/tickets/:ticket_id` with `item`, `PATCH` metadata, `/replies`, and protected `/attachments` resources. These handlers reuse the V1 Store/state machine, permission, audit, notification, and image-validation helpers; do not duplicate ticket rules or make the V2 page call the V1 admin ticket endpoints.
- The V2 admin registration-code route must use `/api/v2/admin/regcodes` with `items`/`pagination`, `/api/v2/admin/regcodes/:code` with `item`, and `/usage` for on-demand usage details. `PATCH`, batch deletion, and usage clearing must retain the V1 validation, storage-mismatch guard, confirmation phrases, atomic Store mutation, reference cleanup, and audit behavior; do not add a browser cache or second registration-code source of truth.
...
- The V2 admin database page uses shared `PageHeader.svelte` and `Panel.svelte` structure. Keep the status, backup, and migration sections independently readable, keep backup history and preview content inside bounded Firefox scroll regions, and preserve the existing SSR/form-action boundary without duplicating database state in browser code.
- The V2 admin configuration page uses shared `PageHeader.svelte` and `Panel.svelte` structure. Keep schema, raw TOML, and backup editing as mutually exclusive modes; preserve secret masking, bounded section/preview scroll regions, server-side field whitelisting, and unsaved-edit protections when changing the layout.
- The V2 `/admin/migration` route uses SSR for migration capability/status reads and form actions for archive preview/import. Export passwords must be sent only in a POST body through `export-download`; the download response is `no-store` and must not be converted to a GET/query-string flow. The complete archive and password must never be retained in browser state. Import confirmation must require a fresh multipart upload plus `IMPORT_TWILIGHT_DATA`; preview summaries may expose only bounded metadata and conflict paths.
- The V2 admin Emby route is SSR and manual-refresh only. It uses `/api/v2/admin/emby/*`; account reads use bounded server pagination/filtering, device/IP audit and ActivityLog reads are separate tab loads, and writes use server form actions. ActivityLog synchronization uses `POST /api/v2/admin/emby/activity-logs/sync`; do not use a state-changing GET query from V2. The V2 namespace reuses the audited V1 handler logic while migration continues. Keep Twilight self-devices excluded, offline device aggregation stable, Emby connectivity probes backend-originated, and diagnostics generic in response envelopes. Broadcast, standalone-account creation, forced password reset, and binding maintenance remain admin-only and audited; generated passwords may appear only in the current action result, never in URLs or persistent browser state.
- The V2 administrator Emby page uses shared `PageHeader.svelte` and `Panel.svelte` structure for its account, device/IP, ActivityLog, and maintenance sections. Keep device/IP and ActivityLog reads manual-only, keep long tables in bounded Firefox scroll regions, and do not reintroduce playback-statistics UI or client polling while changing the layout.
- The V2 admin Bangumi route is SSR and manual-action only. It uses `/api/v2/admin/bangumi/users` plus UID-scoped `/records` and `/logs` resources; user status is server-paginated, playback records and sync logs are fetched only for the selected UID, and per-page record/sync counts must use bounded batch reads. The page reads public feature flags from `/api/v2/system/capabilities`; Bangumi tokens never enter the response. Sync and log clearing use SvelteKit form actions, and Go remains responsible for feature gates, user existence, external sync, persistence, and audit.
- The V2 user announcement route uses the private no-store `/api/v2/announcements` resource for one SSR read of visible and force-read announcements; acknowledgements use `/api/v2/announcements/ack` through a deduplicated server form action. Both wrappers must reuse the shared announcement handlers so visibility, ownership and persistence cannot diverge from V1 compatibility; announcement content remains escaped text unless a reviewed safe renderer is explicitly migrated.
- The V2 score route uses one private no-store `/api/v2/signin/summary` SSR read for signin rules, balance, renewal state, and bounded history. Signin, manual renewal, and the user's auto-renewal preference use `/api/v2/signin`, `/api/v2/signin/renew`, and `/api/v2/signin/preferences` form actions; the V2 handlers must remain thin adapters over the shared Go rules. Manual renewal and automatic-renewal eligibility are separate concepts; both are finally enforced by the existing Go handler and Store transaction.
- The V2 invite route uses private no-store `/api/v2/invite/summary` for one first-view read of invite config, the current relationship projection, direct children, tree summary, and owned invite codes. Creation, physical code deletion, child renewal-code generation, and expired invite detach cleanup use `/api/v2/invite/*` server form actions; the handlers are thin adapters and must not recreate invite eligibility, ownership, or Emby cleanup rules in Svelte. Deleting an invite code does not detach an existing relation, and invite-disabled mode still keeps historical maintenance actions available when the Go handler permits them.
- The V2 admin invite route uses `/api/v2/admin/invite/tree`, `/api/v2/admin/invite/codes`, and `/api/v2/admin/invite/config/schema` for SSR reads. Tree and code reads are private no-store resources; code search/pagination happens on the server. Admin detach, batch maintenance, cascade account actions, and config writes use the corresponding V2 form-action paths while reusing the existing audited Store handlers; do not duplicate invite relationship, renewal, protected-account, or Emby cleanup rules in Svelte.
- The V2 Bangumi route uses private no-store `/api/v2/bangumi/summary` for one first-view read of local sync state, public Bangumi account fields, five bounded collection previews, and recent activity. The Go handler may read the five upstream collection types concurrently, but returns independent partial results and never serializes a user Token. Collection pages use private no-store `/api/v2/bangumi/collections` with server pagination; Bangumi state updates, collection edits, sync, history clearing, token replacement, and mode toggles use `/api/v2/bangumi/*` form actions and the existing Go feature gates. Do not move Bangumi tokens into browser state or let one failed collection request discard successful categories.
