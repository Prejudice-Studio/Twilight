...
- Ticket notification results are outbound side-effects: log success/skip at `Debug` and only failures at `Warn`. Do not log successful sends at `Info` — in JSON-file mode the zap→runtime-log sink persists the whole state document per log line, so per-action success logs would spike write load. See [[project_runtime_log_sink_cost]] and [[project_ticket_notify_async]].
