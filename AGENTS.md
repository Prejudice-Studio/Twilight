...
- **下载触发时机**：`bangumiCollectionsCached`（用户首次拉取收藏时）和 `refreshBangumiCollectionCacheForUser`（调度器每小时刷新）在写入 `BangumiCollectionCache` 后，异步启动 goroutine 下载所有条目的封面图片。封面选择优先级为 `large > common > medium`。
- **本地文件命名**：`{BGMID}.{ext}`（如 `12345.jpg`），BGMID 经过 `isPositiveNumericID` 校验保证为纯正整数，杜绝路径穿越。
- **前端 URL**：所有 Bangumi 封面图片统一使用 `/api/v1/bangumi/cover/:subject_id`（`AuthPublic`，路由在 `routes.go:71`）。Handler `handleBangumiCover`（`bangumi_cover.go:40`）先查本地文件，存在则直接 ServeFile（`Cache-Control: max-age=86400`）；不存在则从 `BangumiSubjectCache` 取原始 `images.large` URL 做 302 跳转回退。
- **安全性**：
