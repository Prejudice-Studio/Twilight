...
| `bangumi_sync_handlers.go` | `handleBangumiSyncStatus`、`handleBangumiSyncTrigger`、`handleBangumiSyncHistory`、`handleBangumiClearHistory`、`handleBangumiMe`、`handleBangumiCollections`、`handleUpdateBangumiCollection`、`handleAdminBangumiUsers`、`handleAdminBangumiRecords`、`handleAdminBangumiSyncUser`、`handleAdminBangumiSyncLogs`、`handleAdminBangumiClearLogs`、`bangumiCollectionsCached`、`refreshBangumiCollectionCacheForUser`、`downloadBangumiCoversForEntries` |
| `bangumi_cover.go` | `handleBangumiCover`、`downloadBangumiCover`、`bangumiCoverFallbackURL`、`isSafeBangumiImageURL`、`isBangumiImageHost` |
...
| `RawBangumiSubjectCache` | — | 按 subject_id 读取原始作品缓存（不含 hydration），用于封面回退 URL 查找 |
...
| `telegramAvailable()` | `handleRegisterBindCode`、`handleUserBindCode` | `TG_NOT_CONFIGURED` |
| `BangumiEnabled` | `handleUpdateMe`(bgm_mode/bgm_token)、`handleBangumiWebhook`、`handleBangumiSyncTrigger`、`handleAdminBangumiSyncUser`、`handleBangumiSyncHistory`、`handleBangumiClearHistory` | `BANGUMI_SYNC_DISABLED` |
...
`Signin.Records` 切片只增不减，随每日签到持续膨胀。`internal/store/signin.go` 的 `AddSigninWithOptions` 在每次追加新记录后自动裁剪超量旧记录：
...
封面图片从 Bangumi CDN 下载到 `uploads/bangumi/{BGMID}.{ext}` 本地缓存，优化前端加载速度：

- **下载触发时机**：`bangumiCollectionsCached`（用户首次拉取收藏时）和 `refreshBangumiCollectionCacheForUser`（调度器每小时刷新）在写入 `BangumiCollectionCache` 后，异步启动 goroutine 下载所有条目的封面图片。封面选择优先级为 `common > large > medium`。
- **本地文件命名**：`{BGMID}.{ext}`（如 `12345.jpg`），BGMID 经过 `isPositiveNumericID` 校验保证为纯正整数，杜绝路径穿越。
- **前端 URL**：所有 Bangumi 封面图片统一使用 `GET /api/v1/bangumi/cover/:subject_id`（`AuthPublic`，路由在 `routes.go:71`）。Handler `handleBangumiCover`（`bangumi_cover.go:40`）先查本地文件，存在则直接 ServeFile（`Cache-Control: max-age=86400`）；不存在则从 `BangumiSubjectCache` 取原始 `images.common` URL 做 302 跳转回退。
- **安全性**：
  - `isSafeBangumiImageURL`（`bangumi_cover.go:27`）校验下载/跳转的 URL scheme 为 `https` 且 host 后缀匹配 `.bgm.tv` 或 `.bangumi.tv`，防止 SSRF 和开放重定向。
  - 下载路径经 `ResolveWithinRoot` 解析，写入前 `MkdirAll` + `Lstat` 防 symlink TOCTOU，文件写入走 `store.WriteFileAtomicSync` 保证原子性。
  - 下载响应体限制 `5<<20`（5MiB），MIME 类型双重校验（`Content-Type` 头 + `http.DetectContentType`），仅允许已知图片格式扩展名。
  - `sharedHTTPClient` 自带的 `sameHostRedirectPolicy` 阻止跨主机重定向泄露请求凭据。
- **去重**：下载前 `Lstat` 检查文件是否已存在，避免重复下载。goroutine 无并发限制（单次最多 1000 条目），每次下载独立 `context.WithTimeout(30s)`。

### Bangumi "看过" 状态处理约定

- `handleUpdateBangumiCollection` 中 `collectType == 2`（看过）始终自动查询 Bangumi 本篇总集数并标记全部剧集为已看，**忽略前端传入的 `ep_status`**。这是因为"看过"语义上等同于"已完成全篇"；如需部分进度应使用"在看"（type=3）。
- 前端 Dashboard 和 Collections 页面在 `editType === 2` 时**不发送** `ep_status` 字段，且切换编辑类型为看过时自动清零进度显示。

### Bangumi 收藏页显示优化约定

Collections 页面（`[type]/page.tsx`）提供以下交互能力：

- **视图切换**：网格（`LayoutGrid`）与列表（`LayoutList`）按钮组，控制在卡片网格与单列行视图之间切换。列表模式下每行展示更紧凑的横向布局（小封面 + 标题/日期/评分/标签/进度 + 状态编辑）。
- **每页数量**：下拉选择器支持 12 / 24 / 48 / 96 条/页，切换时自动回到第 1 页。
- **排序**：支持默认排序 / 进度升序 / 进度降序 / 最近更新 / 评分降序，通过 `useMemo` 客户端侧排序当前页条目。
- **标签筛选**：输入框按标签名筛选当前页条目（大小写不敏感子串匹配），筛选后显示匹配条数。
- **分页导航**：支持首页/上一页/下一页/末页快速跳转。
- 封面图片统一使用 `GET /api/v1/bangumi/cover/:subject_id`（生产环境通过 `API_BASE` 拼接完整后端地址，避免 Next.js 404 拦截）。

### Bangumi Dashboard 最近动态时间线

Dashboard 页面（`page.tsx`）左侧栏除账号信息卡片外，新增「最近动态」时间线卡片：

- 合并五分类缓存（在看/看过/想看/搁置/抛弃）条目，按 `updated_at` 降序排列取前 8 条
- 每条动态显示操作描述（如"看过了 XXX"、"更新 XXX 进度到第 N 话"）与时间
- 条目链接指向对应的 Collections 分类页面
- 列表限高 `420px` 超出滚动，使用 `useMemo` 避免频繁重算
- 时间线使用圆点 + 连接线（`bg-border/10`）的垂直时间轴样式
...
- `internal/store/store.go` 的 `saveLocked` 使用 `json.Marshal` 而非 `json.MarshalIndent` 持久化 state 文件，减少 ~15-30% 体积和序列化耗时（缩进仅在备份/调试时需要，运行时无意义）。
- `internal/store/signin.go` 设定 `maxSigninRecords = 730`，签到记录超量时在 `AddSigninWithOptions` 内联裁剪，不依赖调度任务。
- `internal/store/store.go` 维护 `telegramIDMap` 二级索引（非持久化），避免 `FindUserByTelegramID` 全表扫描。启动时在 `Open`/`OpenPostgres` 中重建，用户 CRUD 操作通过 `maintainTelegramIDIndex` 增量维护。
- 前端语言包（`webui/src/lib/i18n.tsx`）仅 `basic.json` 静态导入作为兜底，其余 locale 文件（`zh-Hant.json`、`en-US.json`、`zh-Hans.json`）通过动态 `import()` 按需加载，Webpack 自动拆分为独立 chunk。
- Zustand store 访问应使用 selector 模式（如 `useAuthStore(s => s.user)` 而非解构整个 store），避免不必要重渲染；`(main)/layout.tsx` 和 `sidebar.tsx` 已按此优化。
- 根页面 `/` 使用 `<Link prefetch={false}>` 替代 `<a>`，避免全页刷新和首屏预取。
- 启动加载器 `BootstrapLoader` 在 `layout.tsx` 中以纯 CSS 全屏 spinner 形式嵌入，HTML 首次渲染立即可见，React 水合后自动淡出，消除白屏期。
...
```bash
go run ./cmd/twilight api --host 0.0.0.0 --port 5000 --config config.toml --debug   # 开发模式
go run ./cmd/twilight api --host 0.0.0.0 --port 5000 --config config.toml            # 生产模式
```
...
- 内置指令列表（`telegram_commands.go`）：`/bind`、`/about`、`/cancel`、`/me`、`/emby`、`/playinfo`、`/resetpwd`、`/delaccount`、`/version`、`/ping`、`/notice`（用户指令）；`/stats`、`/admin`、`/userinfo`、`/twfind`、`/twishelp`、`/banweb`、`/banemby`、`/broadcast`（管理指令）。`/start`、`/help`、`/twihelp`、`/twguser` 走特殊 switch 分支不经过 registry。
- Bot 消息支持 `config.toml` 的 `[Telegram] parse_mode` 配置（`""`/`HTML`/`Markdown`/`MarkdownV2`），作用于所有 `sendMessage` 和 `editMessageText` 调用。JS 沙箱的 `sendMessage()` 额外支持调用时指定 `parse_mode`。
...
- `systemUserLimitReached()`（`internal/api/business.go`）计算用户上限时，不仅统计已有用户，还会将有效注册码（type=1/3）和邀请码的剩余可用名额计入 current 值，避免管理员批量生成卡码后实际用户数超预期。`remainingRegCodeUserSlots` / `remainingInviteUserSlots` 辅助函数用于计算码的剩余名额。
...
### 批量清授权（clearEmbyGrant）语义约定

`handleBatchClearEmbyGrantUnbound` 清理的是**使用了注册码/邀请码但没有正常注册 Emby 账号**的用户的码使用记录：

- 这些用户消耗了码的 `use_count` 配额但未完成 Emby 注册（emby_id 为空），清理后他们可再次使用注册码
...
### Playback Stats Conventions

This section describes the Emby ActivityLog based playback statistics system. It is independent from the Bangumi webhook PlaybackRecords path.

- The feature gate is `EmbyPlaybackStatsEnabled` (default false, config key `[Emby] emby_playback_stats_enabled`). When disabled, all Emby playback stats handlers must return an explicit disabled error; frontend hiding is only a UX layer.
- Activity logs are collected by `fetchAndStoreEmbyActivityLogs` from Emby `/System/ActivityLog/Entries` into `store.EmbyActivityLogs` (max 20000 entries, deduped by Emby log ID).
- `GET /api/v1/admin/emby/activity-logs` (AuthAdmin) returns raw activity logs. It triggers a 2-minute throttled auto refresh by default; `?refresh=1` forces a fetch and `?auto=0` disables the implicit refresh for that request.
- `GET /api/v1/emby/playback-stats` (AuthUser) is limited to the current user. `GET /api/v1/admin/emby/playback-stats` and `/admin/emby/playback-stats/:uid` (AuthAdmin) support global, self, and specific-user views.
- Supported stats params: `scope=self|global|user`, `uid`, `days`, `today=1`, `limit`, `sort=plays|name`, and `refresh=1`. Cross-user and global views must be enforced server-side, not only hidden in the UI.
- This system must not be mixed with `PlaybackRecords` from Bangumi webhook. ActivityLog reliably provides playback event counts, user, item name, and time; it does not reliably provide total playback duration, so `total_duration` is currently a reserved field.
- The frontend `/stats/playback` page auto-loads and polls only while visible, using the selected 30s / 60s / 5m interval. Manual refresh must force-refresh Emby activity logs first.
- The `/admin/emby` activity log panel and device/IP audit panel default to 30s polling, must check document visibility, must expose an off switch, and must clear intervals on unmount.

### Frontend i18n And Stable Layout

- New user-visible copy must use `useI18n().t()`; do not mix hard-coded Chinese or English into locale-enabled pages.
- Add new locale keys only to `basic.json`, `zh-Hant.json`, and `en-US.json`. Do not add keys to sparse `zh-Hans.json`; missing Simplified Chinese keys intentionally fall back to `basic.json`.
- Language-sensitive toolbars, tabs, selects, badges, and buttons should use `i18n-control`, `i18n-label`, `i18n-toolbar`, or equivalent stable sizing so language switching does not change page proportions or cause text overflow.
- Polling UI must show refresh state or last updated time, provide a manual refresh affordance, check page visibility, and clear `setInterval` on unmount.
