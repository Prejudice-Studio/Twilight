...
- Duration is the wall-clock gap between the paired playback start/stop ActivityLog events, so pauses count. Sakura EmbyBoss instead reads `PlayDuration - PauseDuration` from Emby's `PlaybackActivity` table (requires the UserUsageStats plugin). Both are labeled "play duration" but they are not the same number — do not swap one for the other without saying so.
