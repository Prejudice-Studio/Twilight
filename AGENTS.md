...
- Admin device/IP tables, media-request queues, ticket conversations and attachment strips, registration-code usage dialogs, and runtime-log streams must use bounded Firefox scroll regions. Flex conversation panes need `min-h-0` so internal scrolling works at narrow viewport ratios; long tables keep sticky headers and horizontal overflow inside their own region.
