...
async def search_media_by_id(source_type: str, media_id: int):
    ...
    try:
        result = await MediaService.get_by_source_id(
            source=source_type, media_id=media_id, media_type=media_type, include_details=include_details
        )
...
async def search_inventory():
    ...
    try:
        ...
        items = await emby.search_media(search_term=query, include_types=include_types, year=year, limit=limit)
...
async def handle_request_item_by_key(require_key: str):
    ...
    from src.db.user import Role, UserOperate
